...
async function create(data) {
  ...
  const { category, subCategory, gallery, supplier, variationValues, ...otherData } =
    createData;
  console.log('createData', JSON.parse(variationValues));
  ...
  const returnProduct = await db.$transaction(async () => {
    ...
    db.product.update({
      ...
      data: {
        variation: {
          create: JSON.parse(variationValues)[0].variationId,
          attribute: {
            create: JSON.parse(variationValues)[0].attributeId,
          },
        },
      },
    });
